...
import info.woody.so.bean.TrackingBean;
...
public class TrackingController {
	...
	public ResponseDude addRecord(@RequestBody TrackingBean tracking) {
		SqlSession session = sqlSessionFactory.openSession();
		int r = session.insert("insertTracking", tracking);
		session.close();
	...
	public Page<TrackingBean> loadRecords() {
	...
	public Page<TrackingBean> loadRecords(@PathVariable int pageNumber) {
		SqlSession session = sqlSessionFactory.openSession();
		int rowCount = session.selectOne("selectTrackingRowCount");
		...
		List<TrackingBean> trackingList = session.selectList("selectTracking", null, new RowBounds(internalPagination.getOffset(), internalPagination.getLimit()));
		Page<TrackingBean> page = pagination.new Page<TrackingBean>(trackingList);
		session.close();
		return page;
	}

	@RequestMapping(value="/edit", method=RequestMethod.POST, produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseDude updateRecord(@RequestBody TrackingBean tracking) {
		SqlSession session = sqlSessionFactory.openSession();
		session.update("updateTracking", tracking);
		session.close();
	...
	public ResponseDude deleteRecord(@PathVariable int id) {
		SqlSession session = sqlSessionFactory.openSession();
		session.delete("deleteTracking", id);
		session.close();
		return ResponseDude.OK;
	}
