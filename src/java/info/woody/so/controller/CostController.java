...
import info.woody.so.bean.CostBean;
...
public class CostController {
	...
	public ResponseDude addRecord(@RequestBody CostBean cost) {
		SqlSession session = sqlSessionFactory.openSession();
		int r = session.insert("insertCost", cost);
		session.close();
	...
	public Page<CostBean> loadRecords() {
	...
	public Page<CostBean> loadRecords(@PathVariable int pageNumber) {
		SqlSession session = sqlSessionFactory.openSession();
		int rowCount = session.selectOne("selectCostRowCount");
		...
		List<CostBean> costList = session.selectList("selectCost", null, new RowBounds(internalPagination.getOffset(), internalPagination.getLimit()));
		Page<CostBean> page = pagination.new Page<CostBean>(costList);
		session.close();
		return page;
	}

	@RequestMapping(value="/edit", method=RequestMethod.POST, produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseDude updateRecord(@RequestBody CostBean cost) {
		SqlSession session = sqlSessionFactory.openSession();
		session.update("updateCost", cost);
		session.close();
