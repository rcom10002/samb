...
import info.woody.so.bean.FamilyBean;
...
public class FamilyController {
	...
	public ResponseDude addRecord(@RequestBody FamilyBean family) {
		SqlSession session = sqlSessionFactory.openSession();
		int r = session.insert("insertFamily", family);
		session.close();
	...
	public Page<FamilyBean> loadRecords() {
	...
	public Page<FamilyBean> loadRecords(@PathVariable int pageNumber) {
		SqlSession session = sqlSessionFactory.openSession();
		int rowCount = session.selectOne("selectFamilyRowCount");
		...
		List<FamilyBean> familyList = session.selectList("selectFamily", null, new RowBounds(internalPagination.getOffset(), internalPagination.getLimit()));
		Page<FamilyBean> page = pagination.new Page<FamilyBean>(familyList);
		session.close();
		return page;
	}

	@RequestMapping(value="/edit", method=RequestMethod.POST, produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseDude updateRecord(@RequestBody FamilyBean family) {
		SqlSession session = sqlSessionFactory.openSession();
		session.update("updateFamily", family);
		session.close();
	...
	public ResponseDude deleteRecord(@PathVariable int id) {
		SqlSession session = sqlSessionFactory.openSession();
		session.delete("deleteFamily", id);
		session.close();
		return ResponseDude.OK;
	}
