...
import info.woody.so.bean.UserBean;
...
public class UserController {
	...
	public ResponseDude addRecord(@RequestBody UserBean user) {
		SqlSession session = sqlSessionFactory.openSession();
		int r = session.insert("insertUser", user);
		session.close();
	...
	public Page<UserBean> loadRecords() {
	...
	public Page<UserBean> loadRecords(@PathVariable int pageNumber) {
		SqlSession session = sqlSessionFactory.openSession();
		int rowCount = session.selectOne("selectUserRowCount");
		...
		List<UserBean> userList = session.selectList("selectUser", null, new RowBounds(internalPagination.getOffset(), internalPagination.getLimit()));
		Page<UserBean> page = pagination.new Page<UserBean>(userList);
		session.close();
		return page;
	}

	@RequestMapping(value="/edit", method=RequestMethod.POST, produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseDude updateRecord(@RequestBody UserBean user) {
		SqlSession session = sqlSessionFactory.openSession();
